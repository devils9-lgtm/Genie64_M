    ...
    internal class TR_종목정보
    {
        ...
        private static void 처리_ka10100(JsonElement root, string req_stk_cd)
        {
            ...
            if (returnCode == "0")
            {
                ...
                // =========================================================================
                // 📝 [로그 기록] '제주반도체'일 때만 상세 정보 출력
                // =========================================================================
                if (GetSafeString(root, "name") == "제주반도체")
                {
                    string 로그메시지 = $"[제주반도체 분석] 상태: {state} ";

                    // Form1의 Console_print를 호출하여 대시보드에 로그를 남깁니다.
                    // 정적 함수가 아닐 경우 인스턴스 참조가 필요할 수 있습니다.
                    Form1.Console_print(로그메시지);
                }
                if (GetSafeString(root, "name") == "JB금융지주")
                {
                    string 로그메시지 = $"[JB금융지주 분석] 상태: {state}";
