...
namespace 지니
{
    class Tab_ETC_Search
    {
            ...
            var Task = new Task(() =>
            {
                if (Form1.Market_Item_List.ContainsKey(itemcode))
                {
                    ...
                    if (Properties.Settings.Default.CB_매도탐색)
                    {
                        ...
                        if (stockBalanceList.ContainsKey(itemcode))
                        {
                            if (봉확인(Properties.Settings.Default.CBB_Sell_탐색_분봉, Properties.Settings.Default.CBB_Sell_탐색_일봉))
                            {
                                ...
                                double MS_rate = 1 + (Properties.Settings.Default.TB_MS_rate_sell / 100);
                    ...
                    void 검색진입(string itemCode, int 타이머, string 검색식, bool 신규)
                    {
                        ...
                        {
                            검색이탈 탐색 = Form1.form1.검색이탈_LIST.Find(o => o.코드_검색식.Equals(itemCode + "^" + 검색식));

                            if (탐색 == null)
                            {
                                검색이탈 ADD = new 검색이탈(타이머, itemCode + "^" + 검색식, 신규);
                                Form1.form1.검색이탈_LIST.Add(ADD);
                            }
                            else
                            {
                                탐색.타이머 = 타이머;
                            ...
                            var Task1 = new Task(() =>
                            {
                                ...
                                Tab_Watch.Watch_In_Out("I", itemCode, 검색식, DateTime.Now.ToString("HH:mm:ss"));
                            ...
                            Form1.form1.condition_data_Manager.RequestTrData(Task1);
                        });
                    }
                }
            });
            Form1.form1.Real_price_Manager.RequestTrData(Task);
        }
                                                     ...
                                                     string 매수호가수량2, string 매수호가수량3, string 매수호가수량4, string 매수호가수량5, string 매수호가수량6, string 매수호가수량7, string 매수호가수량8, string 매수호가수량9, string 매수호가수량10)
        {
            ...
            Form1.form1.Real_Hoga_Manager.RequestTrData(Task);
        }
    }
}
